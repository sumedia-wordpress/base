...
class Sumedia_Base_Messenger
{
    ...
    public function add_message($type, $message)
    {
        if ($this->is_valid_type($type)) {
            ...
            $_SESSION['sumedia_base_messenger'][$type][] = $message;
        }
    }
    ...
    {
        $html = '';
        if (isset($_SESSION['sumedia_base_messenger'])) {
            if (empty($_SESSION['sumedia_base_messenger'])) {
                return;
            }

            foreach ($_SESSION['sumedia_base_messenger'] as $type => $messages) {
                if (count($messages)) {
    ...
    {
        $_SESSION['sumedia_base_messenger'] = [];
    }
}
